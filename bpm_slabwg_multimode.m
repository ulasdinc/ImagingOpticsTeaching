...
qp_e2 = 2*vp_e(2)/d;        % Wavenumbers q and h 
hp_e2 = 2*up_e(2)/d;
...
B_e2 = 1;
D_e2 = cos(hp_e2*d/2)/exp(-qp_e2*d/2)*B_e2;
C_e2 = D_e2;

mode_0e2 = [D_e2 * exp(qp_e2 * x_clad_1), B_e2 * cos(hp_e2 .* x_core), C_e2 * exp(-qp_e2 * x_clad_2)];
beam_e2 = mode_0e2;
...
figure(),plot(1e+6*x_,(beam_e)); hold on
         plot(1e+6*x_,(beam_o)); hold on
         plot(1e+6*x_,(beam_e2));
         legend('beam e','beam o','beam e2')
         title('3 modes')
         xlabel('x axis [\mum]')
         ylabel('amplitude')
         %print(gcf,'3modes.png','-dpng','-r300');    % save image
...
%         % ### Perturbation by defect ###
%         if z_(index_z) > defect_position - defect_size/2 && z_(index_z) < defect_position + defect_size/2
%             V = (n_core - n_cladding)*(1 + sign((d-defect_depth)/2-abs(x_-defect_depth/2)))/2;
%             disp(index_z)        
%         else
%             V = (n_core - n_cladding)* (double(abs(x_) < d/2)); 
%         end
        
		u3 = u1 .* exp(1i * dz * (nonlinearity .* abs(u1).^2 + V));
		profile_v(:,index_z) = V.';          % [profile_v V.'];
	else
